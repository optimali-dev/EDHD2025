...
for pred, actual, conf in zip(last_y_pred, last_y_test, last_y_pred_std):
    print(f'Predicted: {pred:.2f}, Actual: {actual:.2f}, Conf. Std: {conf:.2f}')
...
print(f'MAE: {sum(mae_list)/len(mae_list):.3f}, MSE: {sum(mse_list)/len(mse_list):.3f}, R2: {sum(r2_list)/len(r2_list):.3f}')
